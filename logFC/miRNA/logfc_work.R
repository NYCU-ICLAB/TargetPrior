...
# setwd("imported file folder")
setwd("D:/logFC/miRNA/")

raw <- read.table (file="TARGET-AML_miRNA.csv",sep=',',  header=T)
input_data <- as.matrix(raw)
input <- t(input_data[,1:471])

eset<-new('ExpressionSet',exprs=as.matrix(input))
design <- model.matrix (~-1+factor(c(rep(1,354),rep(2,62))))
## classification: 1 for 41 normal samples, 2 for 287 tumor samples; Sort patients before processing
colnames(design) <- c('re','normal') 
...
write.table(topTable(P,n=1000000,adjust='BH'),file="miRNA_BHmethod.csv",sep=',',quote=F)
